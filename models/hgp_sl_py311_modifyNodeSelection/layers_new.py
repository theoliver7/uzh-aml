...
class HGPSLPool(torch.nn.Module):
    def __init__(self, in_channels, ratio=0.6, sample=False, sparse=False, sl=True, lamb=1.0, negative_slop=0.2):
    ...
    def forward(self, x, edge_index, edge_attr, batch=None):
        ...
        print("x_information_score.shape", x_information_score.shape)
        score = torch.sum(torch.abs(x_information_score), dim=1) # manhattan distance l1-norm (original)
